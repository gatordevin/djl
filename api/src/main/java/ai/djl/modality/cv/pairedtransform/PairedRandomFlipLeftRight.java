 ...
 */
public class PairedRandomFlipLeftRight implements PairedTransform {

    /** {@inheritDoc} */
    @Override
    public NDArray[] transform(NDArray inputArray, NDArray targetBox) {
        if(new Random().nextBoolean()){
            targetBox.set(new NDIndex(":, 1::2"), array -> array.mul(-1).add(1)); //Only flip x values for horizontal flip
            System.out.println(targetBox.get(":"));
            return new NDArray[] {inputArray.flip(1), targetBox};
        }else{
